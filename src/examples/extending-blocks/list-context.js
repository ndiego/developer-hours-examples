...
function addInspectorControls( BlockEdit ) {
	return ( props ) => {
		...
		const { icon: currentIcon, useIcon } = attributes;
		return (
			<>
				<BlockEdit { ...props } />
				<InspectorControls>
					<PanelBody
						title={ __( 'Settings', 'enable-button-icons' ) }
...
function addListModifications( BlockEdit ) {
	return ( props ) => {
		if ( props.name !== 'core/button' ) {
			return <BlockEdit { ...props } />;
		}

		// Define the new content to be added inside the component
		const newContent = (
			<div className="new-content">
				This is the new content inside BlockListBlock.
			</div>
		);
		//const { icon: currentIcon, useIcon } = attributes;
...
addFilter(
	'editor.BlockEdit',
	'example/add-list-item-modifications',
	addListModifications
);
...
function addIconsToListItemBlocks( element, blockType, attributes ) {
	const { name } = blockType;
	const { isDecorative } = attributes;

	const icon = <span>{ 'Hello there' }</span>;
	const addNewChildFirst = ( element, newChild ) => {
		return cloneElement( element, {
			children: [ newChild ].concat(
	...
	if ( 'core/list-item' === name ) {
		return addNewChildFirst( element, icon );
	}
...
//     'example/add-icon-to-list-item-blocks',
//     addIconsToListItemBlocks
// );
